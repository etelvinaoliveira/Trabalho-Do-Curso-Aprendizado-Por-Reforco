...
def main(_):
    ...
    optimizer = K.optimizers.AdamW(learning_rate=0.025, weight_decay=1e-4, clipnorm=1.0)
        ...
        while not done:
            try:
                ...
                if iter_count % update_after_actions == 0 and len(done_history) > 100:
                    ...
                    # DOUBLE DQN MODIFICATION START
                    # Use main network to select best action for next state
                    next_actions = tf.argmax(q_network.predict(state_next_sample), axis=1)
                    # Use target network to evaluate the selected actions
                    target_q_values = target_network.predict(state_next_sample)
                    # Gather the Q-values for the selected actions
                    target_q_values = tf.gather(target_q_values, next_actions, batch_dims=1)
                    # DOUBLE DQN MODIFICATION END
